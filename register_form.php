...
// Handle debug OTP request
if (isset($_POST['debug_otp']) && isset($_SESSION['pending_registration'])) {
    $pending = $_SESSION['pending_registration'];
    header('Content-Type: application/json');
    echo json_encode([
        'session_id' => session_id(),
        'pending_registration' => $pending,
        'otp_type' => gettype($pending['otp']),
        'otp_value' => $pending['otp'],
        'current_time' => time(),
        'otp_expires' => $pending['otp_expires'],
        'time_remaining' => $pending['otp_expires'] - time()
    ]);
    exit();
}
...
if (isset($_POST['verify_otp']) && isset($_SESSION['pending_registration'])) {
    ...
    error_log("Input OTP: '$input_otp' (Type: " . gettype($input_otp) . ", Length: " . strlen($input_otp) . ")");
    error_log("Stored OTP: '{$pending['otp']}' (Type: " . gettype($pending['otp']) . ", Length: " . strlen($pending['otp']) . ")");
    error_log("Session ID: " . session_id());
    error_log("Raw comparison result: " . var_export($input_otp == $pending['otp'], true));
    error_log("String comparison result: " . var_export((string)$input_otp === (string)$pending['otp'], true));
    ...
    if (time() > $pending['otp_expires']) {
        ...
        // Debug logging (remove this in production)
        error_log("OTP Mismatch - Input: '$input_otp', Expected: '{$pending['otp']}'");
...
<body>
    ...
    <div class="otp-modal" id="otpModal" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(93, 14, 38, 0.8); display: flex; align-items: center; justify-content: center; z-index: 9999;">
        <div style="background: #fff; border-radius: 12px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3); padding: 40px 35px; max-width: 480px; width: 90%; position: relative; animation: slideIn 0.3s ease;">
            ...
            <div style="text-align: center; margin-top: 20px;">
                ...
                <button onclick="debugOTP()" style="background: none; border: none; color: #666; text-decoration: none; font-size: 12px; font-weight: 400; cursor: pointer; padding: 6px 10px; border-radius: 4px; transition: all 0.3s ease; margin-top: 10px; display: block; margin-left: auto; margin-right: auto;" onmouseover="this.style.background='#f0f0f0';" onmouseout="this.style.background='transparent';">
                    🐛 Debug OTP
                </button>
                <button onclick="testFunctions()" style="background: none; border: none; color: #999; text-decoration: none; font-size: 10px; font-weight: 400; cursor: pointer; padding: 4px 8px; border-radius: 3px; transition: all 0.3s ease; margin-top: 5px; display: block; margin-left: auto; margin-right: auto;" onmouseover="this.style.background='#f0f0f0';" onmouseout="this.style.background='transparent';">
                    🧪 Test Functions
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        console.log('=== OTP MODAL SCRIPT LOADED ===');
        console.log('verifyOTP function:', typeof verifyOTP);
        console.log('debugOTP function:', typeof debugOTP);
        console.log('resendOTP function:', typeof resendOTP);
        ...
        function testFunctions() {
            ...
            try {
                showMessage('Test message - functions are working!', 'success');
                console.log('✅ showMessage function working');
            } catch (e) {
                console.error('❌ showMessage function failed:', e);
            }
            
            // Test OTP input
            const otpInput = document.querySelector('input[name="otp"]');
            if (otpInput) {
                console.log('✅ OTP input found:', otpInput.value);
            } else {
            ...
            const debugBtn = document.querySelector('button[onclick="debugOTP()"]');
            const resendBtn = document.querySelector('button[onclick="resendOTP()"]');
            ...
            showMessage('Function test completed! Check console for results.', 'info');
        }

        // Auto-focus on OTP input when modal appears
        document.addEventListener('DOMContentLoaded', function() {
            console.log('=== DOM LOADED - SETTING UP OTP MODAL ===');
            ...
            if (otpInput) {
                ...
                otpInput.value = '';
                console.log('OTP input found and focused');
            } else {
                console.log('OTP input not found');
            }
            ...
            if (otpForm) {
                ...
                // Form submission is now handled by the button onclick
                // No need for additional event listeners
            } else {
                console.log('OTP form not found');
            }
        ...
        function verifyOTP() {
            ...
            console.log('OTP Input value:', otp);
            console.log('OTP Input length:', otp.length);
            console.log('OTP validation test:', /^\d{6}$/.test(otp));
            ...
            console.log('✅ OTP validation passed, proceeding with verification...');
            ...
            fetch(window.location.href, {
                ...
                console.log('✅ Response text received:', response);
                ...
                try {
                    ...
                    console.log('⚠️ Response is not JSON, using fallback parsing');
                    console.log('Response includes login_form.php:', response.includes('login_form.php'));
                    console.log('Response includes OTP expired:', response.includes('OTP expired'));
                    console.log('Response includes Invalid OTP:', response.includes('Invalid OTP'));
                    
                    if (response.includes('login_form.php')) {
                        // Success - redirect to login
                        console.log('✅ SUCCESS: Redirecting to login (fallback)');
        ...
        // Debug function to test OTP verification
        function debugOTP() {
            console.log('=== OTP DEBUG INFO ===');
            const otpInput = document.querySelector('input[name="otp"]');
            if (otpInput) {
                console.log('OTP Input value:', otpInput.value);
                console.log('OTP Input type:', typeof otpInput.value);
                console.log('OTP Input length:', otpInput.value.length);
            }
            ...
            fetch(window.location.href, {
                ...
                try {
                    ...
                    showMessage(`Debug: OTP Type: ${data.otp_type}, Value: ${data.otp_value}`, 'info');
                } catch (e) {
                ...
                showMessage('Debug failed. Check console for errors.', 'error');
            });
        }
