...
foreach ($events as $ev) {
    ...
    // Assign specific colors to each attorney/admin based on actual database users
    if ($attorney_name === $_SESSION['admin_name']) {
        // Admin - Light Red
        $color = '#ff6b6b';
    } elseif ($attorney_name === 'Laica Castillo Refrea') {
        // Attorney 1 - Light Green
        $color = '#51cf66';
    } elseif ($attorney_name === 'Mario Delmo Refrea') {
        // Attorney 2 - Light Blue
        $color = '#74c0fc';
    } elseif ($attorney_name === 'Santiago, Macky Refrea') {
        // Client - Light Orange
        $color = '#ffd43b';
    } elseif ($attorney_name === 'Yuhan Nerfy Refrea') {
        // Employee - Light Violet
        $color = '#da77f2';
    } else {
...
<body>
                    ...
                    <div class="events-grid">
                        ...
                        <div class="event-actions">
                                <div class="status-edit-section">
                                    <select class="event-status-select" data-event-id="<?= $ev['id'] ?>" onchange="updateEventStatus(this)">
    ...
    <style>
        ...
        .event-card[data-attorney="Santiago, Macky Refrea"] {
        ...
        .event-card[data-attorney="Yuhan Nerfy Refrea"] {
        ...
        .event-status-select option[value="Scheduled"] {
            color: #1976d2;
        }

        .event-status-select option[value="Completed"] {
            color: #4caf50;
        }

        .event-status-select option[value="Cancelled"] {
            color: #f44336;
        }

        .event-status-select option[value="Rescheduled"] {
            color: #ff9800;
        }
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventDidMount: function(info) {
                    ...
                    // Apply attorney-based color coding
                    if (info.event.extendedProps.attorneyName) {
                        ...
                        // Set colors based on attorney name
                        if (attorneyName === '<?= $_SESSION['admin_name'] ?? 'Admin' ?>') {
                            backgroundColor = '#ff6b6b'; // Light Red for Admin
                            borderColor = '#ff5252';
                        } else {
                            // Dynamic color assignment for attorneys based on creation order
                            const colorPalette = [
                            ...
                            // Get attorney ID from the event data to determine creation order
                            const attorneyId = info.event.extendedProps.attorneyId || 0;
                            const colorIndex = (attorneyId - 1) % colorPalette.length; // -1 because IDs start at 1
                        ...
                        info.el.style.color = '#333'; // Dark text for readability
        ...
        function updateEventStatus(selectElement) {
            ...
            // First Warning
            if (!confirm(`⚠️ FIRST WARNING: Are you sure you want to change this event status from "${currentStatus}" to "${newStatus}"?\n\nThis action will update the event status immediately.`)) {
            ...
            // Second Warning
            if (!confirm(`🚨 SECOND WARNING: This is your final confirmation!\n\nYou are about to change the event status from "${currentStatus}" to "${newStatus}".\n\nThis action cannot be undone automatically.\n\nClick OK to proceed or Cancel to abort.`)) {
                // Reset to previous value
                selectElement.value = currentStatus;
                return;
            }
        ...
        function initializeStatusSelects() {
            const statusSelects = document.querySelectorAll('.event-status-select');
        ...
        function initializeEditEventHandlers() {
            ...
            if (saveEditEvent) {
                saveEditEvent.addEventListener('click', function() {
