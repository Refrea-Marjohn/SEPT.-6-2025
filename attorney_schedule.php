...
<body>
            ...
            <div class="modal-body">
                <form id="eventForm" class="event-form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <small style="color: #666; font-size: 0.8rem;">Select from your clients for case-related appointments, or any client for free legal advice sessions.</small>
                    </div>
                    <div class="form-group">
    ...
    <style>
        ...
        .upcoming-events-section .events-grid .event-card[data-attorney-name="Laica Castillo Refrea"] {
            ...
            background: linear-gradient(135deg, rgba(81, 207, 102, 0.08) 0%, rgba(81, 207, 102, 0.15) 100%) !important;
        }
        ...
        .upcoming-events-section .events-grid .event-card[data-attorney-name="Santiago, Macky Refrea"] {
        ...
        .upcoming-events-section .events-grid .event-card[data-attorney-name="Mar John Refrea"] {
            ...
            background: linear-gradient(135deg, rgba(255, 107, 107, 0.08) 0%, rgba(255, 107, 107, 0.15) 100%) !important;
        }

        .upcoming-events-section .events-grid .event-card[data-attorney-name="Yuhan Nerfy Refrea"] {
            border-left: 4px solid #da77f2 !important;
            background: linear-gradient(135deg, rgba(218, 119, 242, 0.08) 0%, rgba(218, 119, 242, 0.15) 100%) !important;
        }

        /* Laica Castillo Refrea - Light Green */
        .event-card[data-attorney-name="Laica Castillo Refrea"] {
            border-left: 4px solid #51cf66 !important;
            background: linear-gradient(135deg, rgba(81, 207, 102, 0.08) 0%, rgba(81, 207, 102, 0.15) 100%) !important;
        }

        /* Mario Delmo Refrea - Light Blue */
        .event-card[data-attorney-name="Mario Delmo Refrea"] {
            border-left: 4px solid #74c0fc !important;
            background: linear-gradient(135deg, rgba(116, 192, 252, 0.08) 0%, rgba(116, 192, 252, 0.15) 100%) !important;
        }

        /* Santiago, Macky Refrea - Light Orange */
        .event-card[data-attorney-name="Santiago, Macky Refrea"] {
            border-left: 4px solid #ffd43b !important;
            background: linear-gradient(135deg, rgba(255, 212, 59, 0.08) 0%, rgba(255, 212, 59, 0.15) 100%) !important;
        }

        /* Mar John Refrea - Light Red */
        .event-card[data-attorney-name="Mar John Refrea"] {
            border-left: 4px solid #ff6b6b !important;
            background: linear-gradient(135deg, rgba(255, 107, 107, 0.08) 0%, rgba(255, 107, 107, 0.15) 100%) !important;
        }

        /* Yuhan Nerfy Refrea - Light Violet */
        .event-card[data-attorney-name="Yuhan Nerfy Refrea"] {
            border-left: 4px solid #da77f2 !important;
            background: linear-gradient(135deg, rgba(218, 119, 242, 0.08) 0%, rgba(218, 119, 242, 0.15) 100%) !important;
        }

        /* Default attorney color for any other attorney names */
        .event-card[data-attorney-name]:not([data-attorney-name="Laica Castillo Refrea"]):not([data-attorney-name="Mario Delmo Refrea"]):not([data-attorney-name="Santiago, Macky Refrea"]):not([data-attorney-name="Mar John Refrea"]):not([data-attorney-name="Yuhan Nerfy Refrea"]) {
            border-left: 4px solid #ffd43b !important;
            background: linear-gradient(135deg, rgba(255, 212, 59, 0.08) 0%, rgba(255, 212, 59, 0.15) 100%) !important;
        }
    ...
    <script>
        ...
        var events = <?php echo json_encode(array_map(function($ev) {
            ...
            // Assign specific colors to each attorney based on their name
            switch($attorneyName) {
                case 'Mario Refrea':
                    $eventColor = '#74c0fc'; // Light Blue for Mario Refrea
                    break;
                case 'Ken Anthony Juson':
                    $eventColor = '#51cf66'; // Light Green for Ken Anthony Juson
                    break;
                case 'Mar John Refrea':
                    $eventColor = '#ff6b6b'; // Light Red for Mar John Refrea
                    break;
                case 'Attorney 3':
                    $eventColor = '#da77f2'; // Light Violet for Attorney 3
                    break;
                case 'Attorney 4':
                    $eventColor = '#ffa8a8'; // Light Pink for Attorney 4
                    break;
                default:
                    // Dynamic color assignment for other attorneys
                    $colorPalette = [
                        '#51cf66', // Light Green
                        '#74c0fc', // Light Blue
                        '#ffd43b', // Light Orange
                        ...
                        '#69db7c', // Bright Green
                        '#4dabf7', // Bright Blue
                        '#ffd43b', // Bright Orange
                        '#e599f7', // Bright Violet
                        '#ffb3bf'  // Bright Pink
                    ];
                    
                    // Use attorney ID to determine color
        ...
        function updateEventStatus(selectElement) {
            ...
            switch(newStatus) {
                ...
                case 'Cancelled':
                    ...
                    break;
                default:
                    confirmMessage = `ℹ️ Update event status to "${newStatus}"?\n\nThis will change the event status in the system.`;
                    warningIcon = 'ℹ️';
            }
        ...
        function updateEventCardUI(selectElement, newStatus) {
            ...
            if (attorneyName && attorneyColors[attorneyName]) {
                eventCard.style.borderLeftColor = attorneyColors[attorneyName];
            } else {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventDidMount: function(info) {
                    // Apply attorney-based color coding to events
                    const attorneyName = info.event.extendedProps.attorneyName;
                    const attorneyId = info.event.extendedProps.attorneyId;
                    let backgroundColor = '#6c757d'; // Default gray
                    let borderColor = '#6c757d';
                    
                    console.log('Event:', info.event.title, 'Attorney:', attorneyName, 'ID:', attorneyId);
                    
                    // User-based color coding - consistent color per user
                    if (attorneyName === 'Laica Castillo Refrea') {
                        backgroundColor = '#51cf66'; // Light Green for Laica Castillo Refrea
                        borderColor = '#40c057';
                        console.log('Applying LAICA CASTILLO REFREA colors:', backgroundColor);
                    } else if (attorneyName === 'Mario Delmo Refrea') {
                        backgroundColor = '#74c0fc'; // Light Blue for Mario Delmo Refrea
                        borderColor = '#4dabf7';
                        console.log('Applying MARIO DELMO REFREA colors:', backgroundColor);
                    } else if (attorneyName === 'Santiago, Macky Refrea') {
                        backgroundColor = '#ffd43b'; // Light Orange for Santiago, Macky Refrea
                        borderColor = '#fcc419';
                        console.log('Applying SANTIAGO, MACKY REFREA colors:', backgroundColor);
                    } else if (attorneyName === 'Mar John Refrea') {
                        backgroundColor = '#ff6b6b'; // Light Red for Mar John Refrea
                        borderColor = '#ff5252';
                        console.log('Applying MAR JOHN REFREA colors:', backgroundColor);
                    } else if (attorneyName === 'Yuhan Nerfy Refrea') {
                        backgroundColor = '#da77f2'; // Light Violet for Yuhan Nerfy Refrea
                        borderColor = '#cc5de8';
                        console.log('Applying YUHAN NERFY REFREA colors:', backgroundColor);
                    } else {
                        // Dynamic color assignment for other attorneys based on creation order
                        const colorPalette = [
                            { bg: '#51cf66', border: '#40c057' }, // Light Green (1st attorney)
                            { bg: '#74c0fc', border: '#4dabf7' }, // Light Blue (2nd attorney)
                            { bg: '#ffd43b', border: '#fcc419' }, // Light Orange (3rd attorney)
                            { bg: '#da77f2', border: '#cc5de8' }, // Light Violet (4th attorney)
                            { bg: '#ffa8a8', border: '#ff8787' }, // Light Pink (5th attorney)
                            { bg: '#69db7c', border: '#51cf66' }, // Bright Green (6th attorney)
                            { bg: '#4dabf7', border: '#339af0' }, // Bright Blue (7th attorney)
                            { bg: '#ffd43b', border: '#fcc419' }, // Bright Orange (8th attorney)
                            { bg: '#e599f7', border: '#da77f2' }, // Bright Violet (9th attorney)
                            { bg: '#ffb3bf', border: '#ffa8a8' }  // Bright Pink (10th attorney)
                        ];
                        
                        // Get attorney ID to determine creation order
                        const colorIndex = (attorneyId - 1) % colorPalette.length;
                        const selectedColor = colorPalette[colorIndex];
                        
                        backgroundColor = selectedColor.bg;
                        borderColor = selectedColor.border;
                        
                        console.log('Applying ORDER-BASED colors for', attorneyName, '(ID:', attorneyId, '):', backgroundColor);
                    }
                    
                    // Apply colors to the event element
                    info.el.style.backgroundColor = backgroundColor;
                    info.el.style.borderColor = borderColor;
                    info.el.style.color = '#ffffff'; // White text for readability
                    ...
                    info.el.setAttribute('data-attorney-id', attorneyId);
